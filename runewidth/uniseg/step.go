...
func Step(b []byte, state int) (cluster, rest []byte, boundaries int, newState int) {
	...
	if len(b) <= length { // If we're already past the end, there is nothing else to parse.
		...
		return b, nil, (runeWidth(r, prop) << ShiftWidth), grAny | (prop << shiftPropState)
	}
	...
	for {
		...
		if graphemeBoundary {
			...
			return b[:length], b[length:], boundary, graphemeState | (prop << shiftPropState)
		}
		...
		length += l
		if len(b) <= length {
			return b, nil, (width << ShiftWidth), grAny | (prop << shiftPropState)
		}
	}
}
